...
sed -i 's/GRUB_CMDLINE_LINUX=""/GRUB_CMDLINE_LINUX="console=tty0 rootwait ro fsck.fix=yes fsck.repair=yes net.ifnames=0 noresume apparmor=0 module_blacklist=evbug efi=novamap clk_ignore_unused pd_ignore_unused deferred_probe_timeout=30"/g' /etc/default/grub
